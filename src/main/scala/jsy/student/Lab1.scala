...
object Lab1 extends jsy.util.JsyApplication with jsy.lab1.Lab1Like {
  ...
  def abs(n: Double): Double = return Math.abs(n)
  ...
  def repeat(s: String, n: Int): String = {
   ...
   else if (n < 0) throw new IllegalArgumentException("n cannot be negative")
   return s.concat(repeat(s, n-1))
  }

  def sqrtStep(c: Double, xn: Double): Double = {
    val result = xn - ((Math.pow(xn,2) - c) / (2 * xn))
    return result
  }
  ...
  def sqrtErr(c: Double, x0: Double, epsilon: Double): Double = {
    if (abs((Math.pow(x0,2) - c)) < epsilon) return x0
  ...
  def repOk(t: SearchTree): Boolean = {
    def check(t: SearchTree, min: Int, max: Int): Boolean = t match {
      ...
      case Node(l,d,r) => {
        if ((d <= max) && (d >= min) && check(l,min,d) && check(r,d,max)) {
          /*The checks are for traversal and we are checking d against the mins and maxes*/
          return true
        }
        else return false
      }
    }
    return check(t, Int.MinValue, Int.MaxValue)
  }

  def insert(t: SearchTree, n: Int): SearchTree = t match{
    case Empty => return Node(Empty, n, Empty) //if the tree is empty just create a new node with no children
    case Node(l,d,r) =>{
      if (n >= d) return Node(l,d,insert(r,n)) /* if the value we are trying to insert is greater than or equal to the value
        of the node we are looking at, insert n into the right child */
      else return Node(insert(l,n),d,r) /*if the value we are trying to insert is less than the node we are looking at,
        insert n into the right child*/
    }
  }
  ...
  def deleteMin(t: SearchTree): (SearchTree, Int) = {
    ...
    (t: @unchecked) match {
      ...
      case Node(l, d, r) =>
        val (l1, m) = deleteMin(l) //l1 is mapped to a search tree, m is an int
  ...
  def delete(t: SearchTree, n: Int): SearchTree = t match {
    case Node(l,d,r) => {
      if (n < d) {
        l match {
          case Empty => return t
          case _ => return Node(delete(l,n),d,r)
        }
      }
      else if (n > d){
        r match {
          case Empty => return t
          case _ => return Node(l,d,delete(r,n))
        }
      }
      else {
        r match {
  ...
  def eval(e: Expr): Double = e match {
    ...
    case Binary(op, e1, e2) => {
      op match {
        case Plus => return eval(e1) + eval(e2)
        case Minus => return eval(e1) - eval(e2)
        case Times => return eval(e1) * eval(e2)
        case Div => return eval(e1) / eval(e2)
      }
    }
  }
